...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === "--root") {
      ...
      if (!next)
        throw new Error(
          "--exts requires a comma-separated list like .vue,.js,.css"
        );
      const parts = next
        .split(",")
        .map((s) => s.trim())
      ...
      if (!next)
        throw new Error(
          "--include requires a comma-separated list of directory names"
        );
      const parts = next
        .split(",")
        .map((s) => s.trim())
      ...
      if (!next)
        throw new Error(
          "--exclude requires a comma-separated list of directory names"
        );
      const parts = next
        .split(",")
        .map((s) => s.trim())
...
const pyLineCommentPattern = /(^|\s)#.*$/m;

const htmlExtensions = new Set([".vue", ".html", ".htm"]);
...
function detectCommentTypes(content, extension) {
  ...
  if (cStyleExtensions.has(extension) || extension === ".vue") {
  ...
  if (pythonExtensions.has(extension)) {
    if (pyLineCommentPattern.test(content)) types.add("line");
    // crude detection of triple-quoted docstrings as block
...
function extractComments(content, extension) {
  ...
  // Python triple-quoted strings as block comments
  if (extension === ".py") {
    const reTriple = /("""[\s\S]*?"""|'''[\s\S]*?''')/g;
    let m;
    while ((m = reTriple.exec(content)) !== null) {
      const startIdx = m.index;
      const endIdx = m.index + m[0].length - 1;
      const startLine = indexToLine(startIdx, lineStarts);
      const endLine = indexToLine(endIdx, lineStarts);
...
// Heuristic detection of commented-out code vs prose
function isCodeLikeComment(text, commentType, extension) {
  if (!text) return false;
  let body = String(text);
  // Strip common comment delimiters
  body = body
    .replace(/^\s*\/\//gm, "")
    .replace(/^\s*#/gm, "")
    .replace(/\/\*[\s\S]*?\*\//g, (m) =>
  ...
  const hasParens = /[()]/.test(body);
  const hasSemicolon = /;\s*$|;\s/m.test(body);
  const hasArrow = /=>/.test(body);
  const hasAssignment = /\b\w+\s*=\s*(?![=])/m.test(body);
  const hasComparison = /==={0,1}|!==|<=|>=/.test(body);
  const hasInvocation = /\b[A-Za-z_]\w*\s*\(/.test(body);
  const punctuationDensity =
    (body.match(/[^\w\s]/g) || []).length / Math.max(body.length, 1);
  ...
  if (cStyle.has(extension)) {
    const kw =
      /(\b(function|class|return|if|else|for|while|switch|case|break|continue|try|catch|finally|new|const|let|var|this|import|export)\b)/;
    const cssDecl = /[a-zA-Z_-]+\s*:\s*[^;]+;/; // property: value;
    signals.push(kw.test(body) || cssDecl.test(body));
  }
  if (py) {
    const kw =
      /(\b(def|class|return|if|elif|else|for|while|try|except|finally|with|lambda|yield|async|await|from|import|pass|raise|global|nonlocal|continue|break)\b)/;
    const endsWithColon = /:\s*$/.test(body);
    signals.push(
      kw.test(body) || endsWithColon || hasInvocation || hasAssignment
    );
  }
  if (isHtml) {
    const htmlTag = /<\/?[a-zA-Z][^>]*>/;
    signals.push(htmlTag.test(body));
  }

  // Score threshold
  const score = signals.reduce((acc, v) => acc + (v ? 1 : 0), 0);
  return score >= 2;
}
...
async function run(options) {
  ...
  for await (const filePath of walkDirectory(rootDir, effectiveIgnore)) {
    ...
    if (args.codeOnly) {
      const allComments = extractComments(content, ext);
      const filtered = allComments.filter((c) =>
        isCodeLikeComment(c.text, c.type, ext)
      );
      if (filtered.length === 0) continue;
      itemComments = filtered.map((c) => ({
        type: c.type,
        startLine: c.startLine,
        endLine: c.endLine,
        text: c.text,
      }));
      const types = new Set(itemComments.map((c) => c.type));
      ...
      // If comments aren't needed downstream, we still include them here to support code-only filtering outputs
      if (
        !args.includeLines &&
        ...
        // no-op; comments kept in case other outputs are chosen
      }
      results.push(item);
    } else {
...
function toMarkdown(items, summary, args, rootDir) {
  ...
  lines.push("");
  lines.push(
    `Root: ${args.relative ? path.relative(process.cwd(), rootDir) : rootDir}`
  );
...
async function runCli() {
  ...
  if (args.mdOut) {
    ...
    await fs.promises
      .mkdir(path.dirname(args.mdOut), { recursive: true })
      .catch(() => {});
  ...
  if (args.csvOut) {
    ...
    await fs.promises
      .mkdir(path.dirname(args.csvOut), { recursive: true })
      .catch(() => {});
